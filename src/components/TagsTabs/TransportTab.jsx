...
// 如果你用 Vite 路徑要確認一下是否需要寫 ../../data/...

const TransportTab = ({ data, regionDisplay, advice, loading }) => {
  // 抓 json 裡面對應地區的資料
  const region = regionDisplay || "屏東縣 恆春鎮"; // 預設一個地區
    ...
    <div className="flex flex-col items-center space-y-4 pt-4 text-left w-full max-w-md mx-auto">
      ...
      <div className="bg-white rounded-lg shadow p-3 w-full mt-2">
        ...
        <ul className="space-y-1 text-sm">
          <li>
            <span>最冷月均溫：{jan["1月月均溫_HIST"]}°C → <b className="text-red-600">{jan["1月月均溫_GWL4.0"]}°C</b> <span className="text-gray-500">（升高 {jan["1月月均溫_CHANGE"]}°C）</span></span>
          </li>
          <li>
            <span>最熱月均溫：{jul["7月月均溫_HIST"]}°C → <b className="text-red-600">{jul["7月月均溫_GWL4.0"]}°C</b> <span className="text-gray-500">（升高 {jul["7月月均溫_CHANGE"]}°C）</span></span>
          </li>
          <li>
          ...
          <li>
            <span>強降雨天數：<b>{rain["強降雨天數_GWL4_0"] || "未提供"}</b> 天/年</span>
