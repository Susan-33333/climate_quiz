...
export default function StorySegment({ userData, onNext }) {
  ...
  useEffect(() => {
    ...
    if (projectedAge <= 40) {
      ...
      setBgImage(`${import.meta.env.BASE_URL}mascot/young.jpg`);
    } else if (projectedAge <= 65) {
      setStory("你在壯年努力生活，環境的變遷開始對你產生實際影響...");
      setBgImage(`${import.meta.env.BASE_URL}mascot/adult.jpg`);
    } else {
      setStory("你已步入老年，回顧自己與世界的關係，思考未來的樣貌...");
      setBgImage(`${import.meta.env.BASE_URL}mascot/elder.jpg`);
    }
  }, [userData]);
